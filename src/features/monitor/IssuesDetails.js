...
import { Tabs, Icon, Select,Input, Modal,Breadcrumb,Form,Spin } from 'antd';
...
export class IssuesDetails extends Component {
  ...
  handleIssueStatus(status){
    if(status === 1){
      return <div className="status">{this.props.intl.formatMessage({ id: 'issue_content_status_wait_feed_back' })}</div>
    }else if(status === 2){
      return <div className="status status1">{this.props.intl.formatMessage({ id: 'issue_content_status_wait_confirm' })}</div>
    }else{
      return <div className="status status2">{this.props.intl.formatMessage({ id: 'issue_content_status_confirm' })}</div>
    }
    
  }
  handlefeedbackStatus(status){
    if(status === 2){
      return <div className="status">{this.props.intl.formatMessage({ id: 'issue_content_status_wait_confirm' })}</div>
    }else if(status === 3){
      return <div className="status status2">{this.props.intl.formatMessage({ id: 'issue_content_status_confirm' })}</div>
    }
  ...
  render() {
    ...
    <div>
      <div  className="title_Breadcrumb">
        <Breadcrumb>
          <Breadcrumb.Item><Link to="/monitor/issuesList">{this.props.intl.formatMessage({ id: 'issue_content_h1' })}</Link></Breadcrumb.Item>
          <Breadcrumb.Item>{this.props.intl.formatMessage({ id: 'issue_content_issuesDetails' })}</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      <div>
        ...
        <div className="title_Bottom">
          <div className="people">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_founder' })}： {issueItem.sponsorName}</div>
          <div className="time">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_creationTime' })}： {createTime}</div>
          <div >{this.props.intl.formatMessage({ id: 'issue_content_modalDropdown2' })}： {issueItem.handlerName}</div>
        </div>
      </div>
      <div className="content_Tabs">
      <Tabs defaultActiveKey="1" >
        <TabPane tab={<span><Icon type="align-left" />{this.props.intl.formatMessage({ id: 'issue_content_Details' })}</span>} key="1">
          <div>
            <Form labelCol= {{sm:{span: 8},xs: { span: 24 }}} wrapperCol={{xs: { span: 24 },sm: { span: 16 }}}>
              <Form.Item
              ...
              >             
                  <div style={{paddingTop:"12px"}}>
                    <div className="issue_img">
                      {
                        paths.length ===0 ?this.handleNoImg():paths.map((item,i)=>{
                        return <div>
                            <div key={i} className="imgs" onClick={ ()=>{ this.setState({visible: true,paths:paths})}}>
        ...
        <TabPane tab={<span><Icon type="notification" />{this.props.intl.formatMessage({ id: 'issue_content_issueFeedback' })}</span>} key="2">
            {
              (whoseIusseFeedBack.length === 1 && whoseIusseReplies.length === 0) ||
              (status === 2 && length === 1) ||
              (status === 3 && whoseIusseFeedBack.length > 0)?<div>
            <div className="content_feedback">
              <div className="time">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_creationTime' })}：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; JamesBu</div>
              <div className="people">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_feedbackStaff' })}： {whoseIusseFeedBack[0].nickname}</div>
              <div className="feedback_status">
                <div>{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_feedbackStatus' })}：</div>
              ...
              <Form.Item
                ...
                >             
                  <div style={{paddingTop:"12px"}}>
                    <div className="issue_img">
                      {
                        whoseIusseFeedBack[0].imagePath.length === 0 ?this.handleNoImg():(
                          whoseIusseFeedBack[0].imagePath.map((item, index) =>{
                            return <div className="imgs" onClick={()=>{ this.setState({visible: true,paths:whoseIusseFeedBack[0].imagePath})}}>
          ...
          </div>:this.handleNoData()
            }
          
        </TabPane>
        <TabPane tab={<span><Icon type="inbox" />{this.props.intl.formatMessage({ id: 'issue_content_historyFeedback' })}</span>} key="3">
        <div className="minHeight">
          {
            ...
            status === 3?(
              ...
              historyIusseFeedBack.map((items, i) => {
                return <div key={i}>
                  <div className="content_feedback">
                    <div className="time">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_creationTime' })}：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {items.createTime}</div>
                    <div className="people">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_feedbackStaff' })}： {items.nickname}</div>
                    <div className="feedback_status">
                      <div>{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_feedbackStatus' })}：</div>
                      <div className="status notpass">{this.props.intl.formatMessage({ id: 'issue_statisticsDetails_notPass' })}</div>
                    </div>
                  </div>
                  <Form>
                    <Form.Item
                      ...
                      >             
                        <div style={{paddingTop:"12px"}}>
                          <div className="issue_img">
                            {
                              items.imagePath.length === 0 ?this.handleNoImg():(
                                items.imagePath.map((item, index) =>{
                                  return <div className="imgs" onClick={()=>{ this.setState({visible: true,paths: items.imagePath})}}>
                                    <img alt="example" width="240" height= '120' src={item.url} />
                                  </div>
                                })
                              )
                      ...
                      <Form.Item
                        labelCol= {{sm:{span: 2},xs: { span: 24 }}}
                        wrapperCol={{xs: { span: 24},sm: {span: 21}}}
                        label={this.props.intl.formatMessage({ id: 'issue_content_cause' })}
                      >             
                        <div style={{color:"red"}}>
                          {whoseIusseReplies[i].content?whoseIusseReplies[i].content:<div/>}
                        </div>
                      </Form.Item>
                  </Form>
                </div>
